...
def allowed_pairs(model, key):
    return {(a["from"], a["to"]) for a in model["transitions"][key]["allowed"]}
...
def run(model, data):
    ...
    expected = {"SiteVisits.WorkflowStatus", "VisitActivities.Status", "Photos.ReviewerDecision",
                "Snags.Status", "DocumentJobs.WorkflowStatus", "Documents.ReleaseStatus",
                "NumberRegister.State", "Approvals.Decision", "Projects.Status",
                "InvoiceRequests.FinanceStatus", "InvoiceRequests.QuickBooksStatus"}
    c.check("TRN-01", "Every entity with a lifecycle has a declared transition matrix",
            expected <= set(tr), f"{len(tr)} lifecycles declared: {sorted(tr)}")

    # 2. every declared state is reachable or is an explicit start state
    unreachable = []
    for key, spec in tr.items():
        field = spec["field"]
        col = next(x for x in model["tables"][spec["entity"]]["columns"] if x["name"] == field)
    ...
    sv = allowed_pairs(model, "SiteVisits.WorkflowStatus")
    ...
    docs = allowed_pairs(model, "Documents.ReleaseStatus")
    ...
    nr = allowed_pairs(model, "NumberRegister.State")
    ...
    snag = allowed_pairs(model, "Snags.Status")
    ...
    voiding = [a for a in tr["SiteVisits.WorkflowStatus"]["allowed"] if a["invalidates"]]
    ...
    forbidden = " ".join(model["transitions"]["Photos.ReviewerDecision"]["forbidden"])
    ...
    for v in data["SiteVisits"]:
        states = {a["to"] for a in tr["SiteVisits.WorkflowStatus"]["allowed"]} | {"Draft"}
