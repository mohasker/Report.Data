...
def main():
    ...
    w("")
    w("**Document ID:** AH-SYS-P2A-024 · **Revision:** 2 · **Status:** generated — do not hand-edit")
    w(f"**Generated:** {TODAY} from `model/model.json` by `tools/gen_capture_once.py`")
    w("**Authority:** the owner's operational corrections of 2026-09-11 (decisions D-16 to D-24)")
    w("")
    ...
    w("")
    w("## 3b. Minimum interaction — what the supervisor actually supplies")
    w("")
    ...
    w("")
    w("```")
    w("   " + "  ->  ".join(mi["normal_path"]))
    w("```")
    w("")
    n = len(mi["mandatory_manual_inputs"])
    w(f"**Mandatory manual inputs on that path: {n}.**"
      + (" The supervisor supplies the photographs and nothing else." if n == 0 else ""))
    w("")
    w("### What is populated automatically")
    w("")
    w("| Field | Where the value comes from | When the supervisor is asked |")
    w("|---|---|---|")
    for e in mi["auto_populated"]:
        w(f"| `{e['field']}` | {e['source']} | {e['asks_user']} |")
    w("")
    w("### Required in storage is not the same as required of the supervisor")
    w("")
    w("These fields are required for the record to mean anything, and none of them is a question")
    w("on the normal path:")
    w("")
    for f in mi["required_but_never_typed"]:
        w(f"- `{f}`")
    w("")
    w(f"> {mi['note']}")
    w("")
    w("### What is not required at all")
    w("")
    for t, why in mi["not_required_on_the_normal_path"].items():
        w(f"**`{t}`** — {why}")
        w("")
    w("### Optional, and genuinely optional")
    w("")
    w(", ".join(f"`{f}`" for f in mi["optional_inputs"]) + ".")
    w("")

    w("## 3c. Classification — a proposal is not a fact")
    w("")
    ...
    w("| Column | Standing |")
    w("|---|---|")
    for a in cl["advisory"]:
        w(f"| `{a}` | **Advisory.** Untrusted. Read by the confirmation screen and nothing else |")
    w(f"| `{cl['trusted']}` | **Trusted.** The structured activity a report may use |")
    w(f"| `{cl['state']}` | How far the classification has got |")
    w(f"| `{cl['disposition']}` | What the supervisor did with the proposal |")
    w("")
    w(f"**The rule.** {cl['rule']}")
    w("")
    w(f"**In Quick Share.** {cl['quick_share_behaviour']}")
    w("")
    w("**Barred from reading the candidate:** "
      + ", ".join(cl["may_not_read_candidate"]) + ".")
    w("")
    w("| Classification state | Meaning |")
    w("|---|---|")
    for v in m["enums"]["ClassificationStatus"]["values"]:
        w(f"| `{v['code']}` | {v['description']} |")
    w("")

    w("## 3d. When analysis runs, and when it does not")
    w("")
    pol = cap["ai_analysis_policy"]
    w(f"**{pol['principle']}**")
    w("")
    w("| | " + " | ".join(f"**{k}**" for k in pol["policies"]) + " |")
    w("|---|" + "---|" * len(pol["policies"]))
    for row in ("timing", "unit", "filtered", "why"):
        w(f"| {row.capitalize()} | "
          + " | ".join(pol["policies"][k][row] for k in pol["policies"]) + " |")
    w("")
    w("### Never analysed")
    w("")
    for x in pol["never_analysed"]:
        w(f"- {x}")
    w("")
    w(f"**Still analysed:** {pol['still_analysed']}")
    w("")
    w(f"**{pol['filters_cost_nothing']}**")
    w("")
    w("### Estimated eligibility")
    w("")
    w(f"> **{pol['estimates_only']}**")
    w("")
    est = pol["estimated_eligibility"]
    w("| Class | Estimated share | Basis |")
    w("|---|---|---|")
    for a in est["assumptions"]:
        w(f"| {a['class']} | {a['share'] * 100:.0f}% | {a['basis']} |")
    w(f"| **Eligible** | **{est['eligible_share'] * 100:.0f}%** | the remainder |")
    w("")
    w(f"At the pilot's **{est['captured_per_month_pilot']}** photographs a month, that is about")
    w(f"**{est['eligible_per_month_quick_share']}** analysed under Quick Share and")
    w(f"**{est['eligible_per_month_ai_reviewed']}** under AI Reviewed Share.")
    w("")
    w(f"{est['ai_reviewed_note']}")
    w("")
