...
| **R-38** | A later change quietly makes a **quantitative or contractual field AI-sourced** — a percentage, a quantity, an activity reference — and a model's guess becomes a billable fact. | L | **H** | **High** | Eighteen columns are closed to AI in the canonical model and tested by `CAP-13` and `CAP-14`; the AI's activity suggestion lives in advisory columns that nothing but the confirmation screen reads, and the trusted `ConfirmedActivityTypeID` is human-set. The validation suite fails if any of them is changed. | Architect |
