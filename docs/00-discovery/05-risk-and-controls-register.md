...
**Document ID:** AH-SYS-P0-005 · **Revision:** 0 · **Status:** draft for owner approval
...
| **R-02** | **QuickBooks Online may not be properly supported for a Qatar-registered entity.** Region determines tax handling, numbering and support. | M | H | **High** | Invoicing deferred to Phase 6 and posting to Phase 7. **BQ-05** asked before any accounting design. Financial calculation module kept independent of the accounting product so the target can change without redesign. | GM + accountant | Answer to BQ-05. |
| **R-03** | **AppSheet may not preserve the camera's original file byte-for-byte** (upload-quality settings, EXIF stripping). This touches operating rule 9 directly. | M | H | **High** | Conflict **C-02**. Set image upload quality to the highest available; test on real iOS and Android devices in Phase 2 and record the result; if bytes are not preserved, define "original" as *the file as first received by the system, thereafter never altered*, state that definition openly, and evaluate a separate raw-upload path. **Make no immutability claim before the test exists.** | Architect | Phase 2 device test. |
...
| R-24 | Wrong tax treatment applied. | M | H | High | **BQ-06**; configuration-driven named tax rules; no hard-coded position; accountant's written confirmation filed against A-07. | GM + accountant |
